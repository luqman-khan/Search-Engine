...
public class IndexWriter {
   ...
   */
   private static void buildPostingsFile(String folder, InvertedIndex index,
...
//            for(Long p : postings_array)
//            	postings.add(p.intValue());
            ...
            byte[] docFreqBytes = ByteBuffer.allocate(8)
             .putLong(postings.length).array();
...
//            	System.out.print(docId - lastDocId+" : ");
               byte[] docIdBytes = ByteBuffer.allocate(8)
                .putLong(docId - lastDocId).array(); // encode a gap, not a doc ID
               ...
               postingsFile.write(posFreqBytes, 0, posFreqBytes.length); // write posting frequency
               
               byte[] docRankBytes = ByteBuffer.allocate(8).putLong(0).array(); // calculate rank bytes
...
//            	   System.out.print(pos-lastPos+" , ");
            	   byte[] posBytes = ByteBuffer.allocate(8)
                   .putLong(pos-lastPos).array();
...
//               System.out.println(" ]");
   ...
   private static Long[] buildVocabFile(String folder, String[] dictionary,
    Long[] vocabPositions) {
      ...
      try {
         ...
         vocabList = new OutputStreamWriter(
          new FileOutputStream(new File(folder, "vocab.bin"),false), "ASCII"
         );
